...
if (isset($_POST['Code']) && isset($_POST['Description'])){
    ...
    if (empty($Code)){
        header("Location: add-class.php?error=Course Code is required&$review_data");
        exit();
    } else if(empty($Description)){
        header("Location: add-class.php?error=Description is required&$review_data");
        exit();
       ...
       /* $sql = "SELECT * FROM class WHERE Code ='$Code'";
        ...
        // Course code is correct. So insert review into database
            ...
            $sql2 = "INSERT INTO class(Code, Description)
            ...
            if($result2){
                ...
                header("Location: add-class.php?success=class successfully added.");
                exit();

            } else {

                header("Location: add-class.php?error=Unknown error occurred");
                exit();
