<?php
	//User class to hold all functions needed for the user entity
    class User{
        ...
        //Reads a single user by a given ID from the user
		public function read_single(){
			...
			//retreive row returned from query
			$row = $stmt->fetch(PDO::FETCH_ASSOC);

			//set class memebers to returned row attributes
			$this->ID = $row['ID'];
		...
		//function to create a user given user input
		public function create(){
			...
			//prepare statement
			$stmt = $this->conn->prepare($query);
			//clean data to avoid database problems
			...
			//Auto increment ID by findng max value in database and add 1
			...
			//set ID to be equal to returned max value
			$this->ID = $rowmax['max(ID) + 1']; 
			

			//Default values that are not user controlled 
			$this->Super_flag = 0;
			$this->Client_flag = 1;
			$this->Permissions = "none";
			$this->Date_made = date('Y-m-d');
			
			//binding of parameters to class memebers
			...
			//print error if something goes wrong and return false 
			printf("Error %s. \n", $stmt->error);
			return false;
		}
		...
		public function delete(){
			...
			//clean the data
			$this->ID            = htmlspecialchars(strip_tags($this->ID));
			//bind tha parameter to value entered 
			$stmt->bindParam(':ID', $this->ID);
